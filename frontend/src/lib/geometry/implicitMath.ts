/**
 * Marching cubes — extract a triangle mesh for the isosurface f(x,y,z) = isoValue
 * over a cubic box. Pure math: f is a plain (x,y,z)=>number, so this is testable
 * in Node with no mathjs/Three.js. The 3D viewer compiles a mathjs expression to
 * such a function and passes it in.
 *
 * Used for implicit surfaces that aren't parametrizable — e.g. Fermi surfaces
 * E(k)=E_F (copper's necked sphere), gyroids, metaballs.
 *
 * Tables: standard Marching Cubes edge/triangle tables (Paul Bourke).
 */

// edgeTable[cubeIndex] = 12-bit mask of which cube edges the surface crosses.
...
// triTable[cubeIndex] = list of edge triples forming triangles, terminated by -1.
const TRI_TABLE: number[][] = [
  ...
  [9, 10, 8, 10, 11, 8], [3, 0, 9, 3, 9, 11, 11, 9, 10], [0, 1, 10, 0, 10, 8, 8, 10, 11], [3, 1, 10, 11, 3, 10], [1, 2, 11, 1, 11, 9, 9, 11, 8], [3, 0, 9, 3, 9, 11, 1, 2, 9, 2, 11, 9], [0, 2, 11, 8, 0, 11], [3, 2, 11], [2, 3, 8, 2, 8, 10, 10, 8, 9], [9, 10, 2, 0, 9, 2], [2, 3, 8, 2, 8, 10, 0, 1, 8, 1, 10, 8], [1, 10, 2], [1, 3, 8, 9, 1, 8], [0, 9, 1], [0, 3, 8], [],
]

// 12 cube edges → the two corner indices each connects.
const EDGE_CORNERS: [number, number][] = [
  [0, 1], [1, 2], [2, 3], [3, 0], [4, 5], [5, 6], [6, 7], [7, 4], [0, 4], [1, 5], [2, 6], [3, 7],
]

// Corner offsets within a unit cell (matches MC convention).
...
export interface MarchingCubesResult {
  /** Flat triangle vertices: [x0,y0,z0, x1,y1,z1, ...]; every 9 floats = 1 triangle. */
...
/**
 * Extract the isosurface f(x,y,z) = isoValue over the cube [min,max]^3.
 * @param f scalar field
 * @param isoValue threshold
 * @param bounds [min, max] cube extent per axis
 * @param resolution cells per axis (capped at 64). Vertices sampled = (res+1)^3.
 */
export function marchingCubes(
  ...
  const res = Math.max(2, Math.min(64, Math.floor(resolution)))
  const [lo, hi] = bounds
  const step = (hi - lo) / res
  const N = res + 1

  // Pre-sample the field on the (res+1)^3 grid (each corner evaluated once).
  ...
  for (let i = 0; i < N; i++) {
    ...
    for (let j = 0; j < N; j++) {
      ...
      for (let k = 0; k < N; k++) {
        const z = lo + k * step
        const v = f(x, y, z)
        grid[(i * N + j) * N + k] = isFinite(v) ? v : isoValue + 1e9
      }
    }
  }
  ...
  const edgeV: [number, number, number][] = Array.from({ length: 12 }, () => [0, 0, 0])
  ...
  for (let i = 0; i < res; i++) {
    for (let j = 0; j < res; j++) {
      for (let k = 0; k < res; k++) {
        let cubeIndex = 0
        for (let c = 0; c < 8; c++) {
          const [oi, oj, ok] = CORNER_OFFSETS[c]
          const val = at(i + oi, j + oj, k + ok)
          corner[c] = val
          cx[c] = lo + (i + oi) * step
          cy[c] = lo + (j + oj) * step
          cz[c] = lo + (k + ok) * step
        ...
        for (let e = 0; e < 12; e++) {
          if (edges & (1 << e)) {
            ...
            const vb = corner[b]
            const denom = vb - va
            const t = Math.abs(denom) > 1e-12 ? (isoValue - va) / denom : 0.5
            edgeV[e][0] = lerp(t, cx[a], cx[b])
            edgeV[e][1] = lerp(t, cy[a], cy[b])
            edgeV[e][2] = lerp(t, cz[a], cz[b])
          }
        }
