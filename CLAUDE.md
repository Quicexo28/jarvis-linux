...
- Push-to-talk: hold `Ctrl+Alt+V` (Hyprland bind → `/api/jarvis/ptt/start|stop` → skill-bus `ptt_set`) or the in-app key (`pttKey` in jarvisStore, default Space, `usePushToTalk`). PTT turns send `ptt: true` and bypass the speaker-confidence + mute + intent gates (`pttState.js`, `_resolveSpeakerMode`). With `pttEnabled`, continuous listening is off — mic streams only while held.
