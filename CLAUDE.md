...
- Push-to-talk: hold `Ctrl+Alt+V` (Hyprland bind → `/api/jarvis/ptt/start|stop` → skill-bus `ptt_set`) or the in-app combo (`pttKey` in jarvisStore, default `Ctrl+Alt+KeyV` to mirror the global bind, `usePushToTalk`). PTT turns send `ptt: true` and bypass the speaker-confidence + mute + intent gates (`pttState.js`, `_resolveSpeakerMode`). With `pttEnabled`, continuous listening is off — mic streams only while held.
- First-boot speaker enrollment: `EnrollmentWizard.tsx` overlays AWAKE until a speaker with ≥1 sample exists (4 guided phrases → `/api/speaker-id/*`, flag `jarvis.enrollment.done.v1`).
