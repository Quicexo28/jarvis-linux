...
#
# Genera: $HOME\Desktop\jarvis-migration.zip
...
Write-Host "[pack-migration] Empacando migración Jarvis → Linux..." -ForegroundColor Cyan
...
if (-not (Test-Path $PortableVault)) {
    Write-Error "No existe $PortableVault — ejecuta primero: node backend/scripts/make-portable.js"
    exit 1
}
...
try {
    ...
    # 3. Obsidian vault (Jarvis-Vault)
    if (Test-Path $VaultPath) {
        $VaultDest = Join-Path $TmpDir "Jarvis-Vault"
        Copy-Item -Recurse -Force $VaultPath $VaultDest
        Write-Host "  [ok] Jarvis-Vault/ ($((Get-ChildItem $VaultDest -Recurse -File).Count) archivos)"
    } else {
        Write-Host "  [warn] Jarvis-Vault no encontrado en $VaultPath — ajusta -VaultPath"
    }
    ...
    Write-Host "  1. Transfiere $OutputZip al PC Linux (SCP, USB, Syncthing, etc.)"
