#!/usr/bin/env node
/**
 * Unlock the portable owner vault on this PC (one-time per machine).
 *
 * Decrypts data/jarvis-portable.enc with the owner password and writes the
 * per-machine caches (secrets.local.enc + owner_voiceprint.enc) so the app boots
 * hands-free afterward. Headless alternative to the in-app unlock modal — useful
 * on a fresh PC before first launch.
 *
...
import { dpapiEncrypt, dpapiEncryptRaw } from '../src/lib/platformCrypto.js'
...
async function main() {
  if (!existsSync(PORTABLE)) {
    console.error(`No existe ${PORTABLE} — copia la bóveda portable a esta PC primero.`)
    process.exit(1)
  }
  ...
  console.log('Desbloqueado en esta PC. Cachés DPAPI locales escritas. Inicia Jarvis normal.')
}
