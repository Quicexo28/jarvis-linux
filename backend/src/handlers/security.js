 ...
 *
 * On a new PC the owner vault (data/jarvis-portable.enc) is locked until the
 * owner enters the password once. Unlocking decrypts the vault (voiceprint +
 * secrets) and writes per-machine caches so subsequent boots are hands-free.
...
import { dpapiEncrypt, dpapiEncryptRaw } from '../lib/platformCrypto.js'
...
function isUnlocked() {
  // Local encrypted secrets present (written at unlock or by make-portable) AND
  // the code password hash is in the environment.
...
export function handleSecurityStatus(_req, res) {
  ...
  return json(res, 200, {
    ...
    hasPortable: !!blob,
    unlocked: isUnlocked(),
    // The renderer prompts for the password when there's a vault but no local
    // unlock yet (typically a fresh install on a new PC).
...
export async function handleSecurityUnlock(req, res) {
  ...
  try {
    ...
    // Write per-machine caches so future boots are hands-free.
    ...
    // Apply secrets to the live process now (tokens, password hash) — existing
    // env wins, so only fill what's missing.
    for (const [k, v] of Object.entries(secrets)) {
      if (k.startsWith('_') || typeof v !== 'string' || !v) continue
      if (!process.env[k]) process.env[k] = v
    }
    // Make the STT service reload the owner voiceprint it now has on disk.
    try {
