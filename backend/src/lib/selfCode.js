...
export async function gitCheckpoint({ message } = {}) {
  ...
  await git('add -A', cwd)
  // --allow-empty so a checkpoint always produces a sha even with no changes.
...
/**
 * Apply backend code changes by exiting with code 99, which Electron's
 * supervisor treats as a restart request (see electron/main.js). Caller MUST
 * send its HTTP response before this fires.
 */
